...
static void display_vni(struct vty *vty, struct bgpevpn *vpn, json_object *json)
{
	...
	if (json) {
		...
		json_object_string_addf(json, "originatorIp", "%pIA", &vpn->originator_ip);
...
static void show_vni_routes(struct bgp *bgp, struct bgpevpn *vpn, struct vty *vty, int type,
			    bool mac_table, struct ipaddr *vtep_ip, json_object *json, int detail)
{
	struct bgp_dest *dest;
	struct bgp_path_info *pi;
...
static void show_vni_routes_hash(struct hash_bucket *bucket, void *arg)
{
	...
	show_vni_routes(wctx->bgp, vpn, wctx->vty, wctx->type, wctx->mac_table, &wctx->vtep_ip,
...
static void show_vni_routes_all_hash(struct hash_bucket *bucket, void *arg)
{
	...
	show_vni_routes(wctx->bgp, vpn, wctx->vty, 0, false, &wctx->vtep_ip, json_vni, wctx->detail);
			     ...
			     json_object *json)
{
	...
	if (json) {
		...
		json_object_string_addf(json_vni, "originatorIp", "%pIA", &bgp->originator_ip);
 ...
 */
static void evpn_show_routes_vni_all(struct vty *vty, struct bgp *bgp, int type, bool mac_table,
				     const union sockunion *vtep_ip, json_object *json, int detail)
{
	uint32_t num_vnis;
...
static void evpn_show_routes_vni_all_type_all(struct vty *vty, struct bgp *bgp,
					      const union sockunion *vtep_ip, json_object *json,
					      int detail)
{
	uint32_t num_vnis;
	struct vni_walk_ctx wctx;
 ...
 */
static void evpn_show_route_vni_multicast(struct vty *vty, struct bgp *bgp, vni_t vni,
					  struct ipaddr *orig_ip, json_object *json)
{
	struct bgpevpn *vpn;
 ...
 */
static void evpn_show_routes_vni(struct vty *vty, struct bgp *bgp, vni_t vni, int type,
...
DEFPY(show_bgp_l2vpn_evpn_route_vni, show_bgp_l2vpn_evpn_route_vni_cmd,
      ...
      " [<type <ead|1|macip|2|multicast|3> | vtep <A.B.C.D|X:X::X:X>$vtep_ip>] [json]",
      SHOW_STR BGP_STR L2VPN_HELP_STR EVPN_HELP_STR EVPN_RT_HELP_STR
      "VXLAN Network Identifier\n"
      "VNI number\n" EVPN_TYPE_HELP_STR EVPN_TYPE_1_HELP_STR EVPN_TYPE_1_HELP_STR
 ...
 */
DEFPY(show_bgp_l2vpn_evpn_route_vni_all, show_bgp_l2vpn_evpn_route_vni_all_cmd,
 ...
 */
DEFPY(show_bgp_vni_all, show_bgp_vni_all_cmd,
      "show bgp vni all [vtep <A.B.C.D|X:X::X:X>$addr] [detail$detail] [json$uj]",
      SHOW_STR BGP_STR VNI_HELP_STR VNI_ALL_HELP_STR VTEP_HELP_STR VTEP_IP_HELP_STR
	      VTEP_IPV6_HELP_STR DETAIL_HELP_STR JSON_STR)
{
	struct bgp *bgp;
	json_object *json = NULL;
 ...
 */
DEFPY(show_bgp_vni_all_ead, show_bgp_vni_all_ead_cmd,
      "show bgp vni all type <1|ead> [vtep <A.B.C.D|X:X::X:X>$addr] [<detail$detail|json$uj>]",
      SHOW_STR BGP_STR VNI_HELP_STR VNI_ALL_HELP_STR EVPN_TYPE_HELP_STR EVPN_TYPE_1_HELP_STR
	      EVPN_TYPE_1_HELP_STR VTEP_HELP_STR VTEP_IP_HELP_STR VTEP_IPV6_HELP_STR DETAIL_HELP_STR
		      JSON_STR)
{
	struct bgp *bgp;
	json_object *json = NULL;
 ...
 */
DEFPY(show_bgp_vni_all_macip_mac, show_bgp_vni_all_macip_mac_cmd,
      "show bgp vni all type <2|macip> mac [vtep <A.B.C.D|X:X::X:X>$addr] [<detail$detail|json$uj>]",
      SHOW_STR BGP_STR VNI_HELP_STR VNI_ALL_HELP_STR EVPN_TYPE_HELP_STR EVPN_TYPE_2_HELP_STR
	      EVPN_TYPE_2_HELP_STR
      "MAC Table\n" VTEP_HELP_STR VTEP_IP_HELP_STR VTEP_IPV6_HELP_STR DETAIL_HELP_STR JSON_STR)
{
	struct bgp *bgp;
	json_object *json = NULL;
 ...
 */
DEFPY(show_bgp_vni_all_macip_ip, show_bgp_vni_all_macip_ip_cmd,
 ...
 */
DEFPY(show_bgp_vni_all_imet, show_bgp_vni_all_imet_cmd,
      "show bgp vni all type <3|multicast> [vtep <A.B.C.D|X:X::X:X>$addr] [<detail$detail|json$uj>]",
      SHOW_STR BGP_STR VNI_HELP_STR VNI_ALL_HELP_STR EVPN_TYPE_HELP_STR EVPN_TYPE_3_HELP_STR
	      EVPN_TYPE_3_HELP_STR VTEP_HELP_STR VTEP_IP_HELP_STR VTEP_IPV6_HELP_STR DETAIL_HELP_STR
		      JSON_STR)
{
	struct bgp *bgp;
	json_object *json = NULL;
...
DEFPY(show_bgp_vni, show_bgp_vni_cmd,
      ...
      SHOW_STR BGP_STR VNI_HELP_STR VNI_NUM_HELP_STR VTEP_HELP_STR VTEP_IP_HELP_STR
	      VTEP_IPV6_HELP_STR JSON_STR)
{
	struct bgp *bgp;
	json_object *json = NULL;
 ...
 */
DEFPY(show_bgp_vni_ead, show_bgp_vni_ead_cmd,
      "show bgp vni " CMD_VNI_RANGE "$vni type <1|ead> [vtep <A.B.C.D|X:X::X:X>$addr] [json$uj]",
      SHOW_STR BGP_STR VNI_HELP_STR VNI_NUM_HELP_STR EVPN_TYPE_HELP_STR EVPN_TYPE_1_HELP_STR
	      EVPN_TYPE_1_HELP_STR VTEP_HELP_STR VTEP_IP_HELP_STR VTEP_IPV6_HELP_STR JSON_STR)
{
	struct bgp *bgp;
	json_object *json = NULL;
 ...
 */
DEFPY(show_bgp_vni_macip_mac, show_bgp_vni_macip_mac_cmd,
      "show bgp vni " CMD_VNI_RANGE
      "$vni type <2|macip> mac [vtep <A.B.C.D|X:X::X:X>$addr] [json$uj]",
      SHOW_STR BGP_STR VNI_HELP_STR VNI_NUM_HELP_STR EVPN_TYPE_HELP_STR EVPN_TYPE_2_HELP_STR
	      EVPN_TYPE_2_HELP_STR
      "MAC Table\n" VTEP_HELP_STR VTEP_IP_HELP_STR VTEP_IPV6_HELP_STR JSON_STR)
{
	struct bgp *bgp;
	json_object *json = NULL;
 ...
 */
DEFPY(show_bgp_vni_macip_ip, show_bgp_vni_macip_ip_cmd,
      "show bgp vni " CMD_VNI_RANGE
      "$vni type <2|macip> ip [vtep <A.B.C.D|X:X::X:X>$addr] [json$uj]",
      SHOW_STR BGP_STR VNI_HELP_STR VNI_NUM_HELP_STR EVPN_TYPE_HELP_STR EVPN_TYPE_2_HELP_STR
	      EVPN_TYPE_2_HELP_STR
      "IP Table\n" VTEP_HELP_STR VTEP_IP_HELP_STR VTEP_IPV6_HELP_STR JSON_STR)
{
 ...
 */
DEFPY(show_bgp_vni_imet, show_bgp_vni_imet_cmd,
      "show bgp vni " CMD_VNI_RANGE
      "$vni type <3|multicast> [vtep <A.B.C.D|X:X::X:X>$addr] [json$uj]",
      SHOW_STR BGP_STR VNI_HELP_STR VNI_NUM_HELP_STR EVPN_TYPE_HELP_STR EVPN_TYPE_3_HELP_STR
	      EVPN_TYPE_3_HELP_STR VTEP_HELP_STR VTEP_IP_HELP_STR VTEP_IPV6_HELP_STR JSON_STR)
{
	struct bgp *bgp;
	json_object *json = NULL;
      ...
      "IP Table\n" IP_ADDR_STR IP6_ADDR_STR JSON_STR)
{
	...
	if (sockunion_family(ip) == AF_INET) {
		...
		memcpy(&ip_addr.ipaddr_v6, &ip->sin6.sin6_addr, sizeof(struct in6_addr));
	}
