...
TRACEPOINT_EVENT(
	...
	TP_ARGS(esi_t *, esi, vni_t, vni,
		...
		struct ipaddr *, vtep_v6),
	TP_FIELDS(
		ctf_array(unsigned char, esi, esi, sizeof(esi_t))
		ctf_integer(vni_t, vni, vni)
		ctf_integer(uint8_t, route_type, route_type)
		ctf_integer_network_hex(unsigned int, vtep_v4, vtep_v4.s_addr)
		ctf_array(unsigned char, vtep_v6, vtep_v6, sizeof(struct ipaddr))
	)
)
...
TRACEPOINT_EVENT(
	frr_bgp,
	handle_maint_mode_zrecv,
	TP_ARGS(bool, enter_maint),
	TP_FIELDS(
		ctf_integer(bool, enter_maint, enter_maint)
	)
)
TRACEPOINT_LOGLEVEL(frr_bgp, handle_maint_mode_zrecv, TRACE_INFO)
 ...
 * state is decoded via bfd_state enum: 0=UNKNOWN, 1=ADMIN_DOWN, 2=DOWN, 3=INIT, 4=UP
 */
TRACEPOINT_EVENT(
	frr_bgp,
 ...
 * Traced when BGP processes fast shutdown and deregisters all BFD sessions
 */
TRACEPOINT_EVENT(
	frr_bgp,
	bfd_fast_shutdown,
