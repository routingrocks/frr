				   ...
				   int set)
{
	...
	/* Prevent max-paths configuration for SAFI_UNREACH (hardcoded to 1) */
	if (safi == SAFI_UNREACH) {
		vty_out(vty,
			"%% maximum-paths is not configurable for unreachability SAFI (fixed at 1)\n");
		return CMD_WARNING_CONFIG_FAILED;
	}
...
{
	/* Don't write maximum-paths for SAFI_UNREACH (hardcoded to 1, not configurable) */
	if (safi == SAFI_UNREACH)
		return;
      ...
      NO_STR BGP_STR "Attach route origin ext community\n")
{
	...
	/* Allow advertise-origin for SAFI_UNICAST and SAFI_UNREACH */
	if (safi != SAFI_UNICAST && safi != SAFI_UNREACH)
		return CMD_SUCCESS;
...
DEFPY(bgp_per_src_nhg_convergence_timer, bgp_per_src_nhg_convergence_timer_cmd,
      "bgp per-source-nhg convergence {interval (5-30000)$interval|maxtime (1000-120000)$maxtime}",
      BGP_STR
      "Per Source NHG settings\n"
      "Convergence settings\n"
      "Interval setting\n"
      "Interval in milliseconds at which the SOO timer triggers for per-source NHG (Default: 50 msec)\n"
      "Maxtime setting\n"
      "Maximum duration in milliseconds after which the SOO timer stops if convergence is not achieved (Default: 20000 msec)\n")
{
	VTY_DECLVAR_CONTEXT(bgp, bgp);
	...
	if (new_interval >= new_maxtime) {
		vty_out(vty, "%% Error: interval (%d ms) must be less than maxtime (%d ms)\n",
			new_interval, new_maxtime);
		return CMD_WARNING;
	}

	/* Check if values are unchanged */
	if ((!interval || interval == bgp->per_src_nhg_convergence_timer) &&
	    (!maxtime || maxtime == bgp->per_src_nhg_convergence_max_time))
		return CMD_SUCCESS;
...
DEFPY(no_bgp_per_src_nhg_convergence_timer, no_bgp_per_src_nhg_convergence_timer_cmd,
      "no bgp per-source-nhg convergence [interval [(5-30000)]] [maxtime [(1000-120000)]]",
		     ...
		     "Maxtime in milliseconds\n")
{
	...
	if (argv_find(argv, argc, "interval", &idx))
		reset_interval = true;
	if (argv_find(argv, argc, "maxtime", &idx))
		reset_maxtime = true;

	/* If neither specified, reset both */
	if (!reset_interval && !reset_maxtime) {
		reset_interval = true;
		reset_maxtime = true;
	}

	/* Determine what the new values will be after reset */
	new_interval = reset_interval ? BGP_PER_SRC_NHG_SOO_TIMER_WHEEL_PERIOD
				      : bgp->per_src_nhg_convergence_timer;
	new_maxtime = reset_maxtime ? BGP_PER_SRC_NHG_SOO_TIMER_TIMEOUT
				    : bgp->per_src_nhg_convergence_max_time;

	/* Validate: interval must be less than max_time */
	if (new_interval >= new_maxtime) {
		vty_out(vty,
			"%% Error: Cannot reset - interval (%d ms) must be less than maxtime (%d ms)\n",
			new_interval, new_maxtime);
		vty_out(vty, "%% Current: interval=%d ms, maxtime=%d ms\n",
			bgp->per_src_nhg_convergence_timer, bgp->per_src_nhg_convergence_max_time);
		return CMD_WARNING;
	}
	...
	if (reset_interval && reset_maxtime) {
		...
		if (bgp->per_src_nhg_convergence_timer == BGP_PER_SRC_NHG_SOO_TIMER_WHEEL_PERIOD)
			return CMD_SUCCESS;
	} else if (reset_maxtime) {
		if (bgp->per_src_nhg_convergence_max_time == BGP_PER_SRC_NHG_SOO_TIMER_TIMEOUT)
			return CMD_SUCCESS;
	}

	/* Reset maxtime if requested */
	if (reset_maxtime)
		bgp->per_src_nhg_convergence_max_time = BGP_PER_SRC_NHG_SOO_TIMER_TIMEOUT;
...
static void bgp_listen_range_config_write(struct vty *vty, struct bgp *bgp, json_object *json)
{
	afi_t afi;
	struct prefix *range;
	struct peer_group *group;
	...
	if (json) {
		if (bgp->dynamic_neighbors_limit != BGP_DYNAMIC_NEIGHBORS_LIMIT_DEFAULT)
			json_object_int_add(json, "dynamicNeighLimit", bgp->dynamic_neighbors_limit);
...
static void bgp_timer_config_write(struct vty *vty, struct bgp *bgp, json_object *json)
{
	if (json) {
		/* Confederation identifier info */
		if (CHECK_FLAG(bgp->config, BGP_CONFIG_CONFEDERATION))
			json_object_int_add(json, "confederationId", bgp->confed_id);
		/* Confederation peer info */
		...
		if (bgp->default_keepalive != SAVE_BGP_KEEPALIVE ||
		    bgp->default_holdtime != SAVE_BGP_HOLDTIME)
			vty_out(vty, "BGP Timers Keepalive: %u Holdtime: %u\n",
...
static void bgp_route_target_config_write(struct vty *vty, struct bgp *bgp, json_object *json)
{
			   ...
			   const char *type)
{
	...
	if (json) {
		...
		/* BGP timers & Confederation identifier, peer info*/
		bgp_timer_config_write(vty, bgp, json);

		/* listen range and limit for dynamic BGP neighbors info */
		bgp_listen_range_config_write(vty, bgp, json);

		/* import and export route-target info */
		bgp_route_target_config_write(vty, bgp, json);
		...
		for (afi = AFI_IP; afi <= AFI_IP6; afi++) {
			...
			json_object_string_add(json_addr, "afi", get_afi_safi_str(afi, safi, false));
			json_object_boolean_add(json_addr, "advertiseOrigin",
			...
			for (dest = bgp_table_top(bgp->route[afi][safi]); dest;
				...
				if (bgp_static->user_configured)
					json_object_object_addf(json_network, json_static, "%pFX",
								p);
			}
			...
			for (dest = bgp_table_top(bgp->aggregate[afi][safi]); dest;
				...
				json_object_object_addf(json_aggregate, json_agg, "%pFX", p);
			}
			json_object_object_add(json_addr, "aggregateAddress", json_aggregate);
				       ...
				       const char *type, bool use_vrf)
{
	...
	if (use_vrf) {
		...
		if (json) {
			...
			/* BGP timers & Confederation identifier, peer 	info*/
			bgp_timer_config_write(vty, bgp, NULL);

			/* listen range and limit for dynamic BGP neighbors info */
			bgp_listen_range_config_write(vty, bgp, NULL);

			/* import export route-target info */
			bgp_route_target_config_write(vty, bgp, NULL);
			...
			for (afi = AFI_IP; afi <= AFI_IP6; afi++) {
				...
				vty_out(vty, "   BGP advertise origin: %s\n",
					CHECK_FLAG(bgp->per_src_nhg_flags[afi][safi],
						   BGP_FLAG_ADVERTISE_ORIGIN)
						? "YES"
						: "NO");
				vty_out(vty, "   BGP nhg per origin: %s\n",
					CHECK_FLAG(bgp->per_src_nhg_flags[afi][safi],
						   BGP_FLAG_NHG_PER_ORIGIN)
						? "YES"
						: "NO");
	...
	for (ALL_LIST_ELEMENTS(bm->bgp, mnode, mnnode, bgp)) {
		...
		if (bgp->per_src_nhg_convergence_timer != BGP_PER_SRC_NHG_SOO_TIMER_WHEEL_PERIOD ||
			...
			bool show_maxtime = (bgp->per_src_nhg_convergence_max_time !=
					     BGP_PER_SRC_NHG_SOO_TIMER_TIMEOUT);

			if (show_interval && show_maxtime) {
				vty_out(vty,
					" bgp per-source-nhg convergence interval %d maxtime %d\n",
					bgp->per_src_nhg_convergence_timer,
					bgp->per_src_nhg_convergence_max_time);
			} else if (show_interval) {
				vty_out(vty, " bgp per-source-nhg convergence interval %d\n",
					bgp->per_src_nhg_convergence_timer);
			} else if (show_maxtime) {
				vty_out(vty, " bgp per-source-nhg convergence maxtime %d\n",
					bgp->per_src_nhg_convergence_max_time);
			}
		}
...
void bgp_vty_init(void)
{
	...
	install_element(BGP_IPV4U_NODE, &neighbor_allowas_in_route_map_cmd);
	install_element(BGP_IPV4U_NODE, &no_neighbor_allowas_in_route_map_cmd);
	install_element(BGP_IPV6U_NODE, &neighbor_allowas_in_route_map_cmd);
	install_element(BGP_IPV6U_NODE, &no_neighbor_allowas_in_route_map_cmd);
	...
	install_element(BGP_IPV4U_NODE, &bgp_advertise_origin_cmd);
	install_element(BGP_IPV6U_NODE, &bgp_advertise_origin_cmd);
