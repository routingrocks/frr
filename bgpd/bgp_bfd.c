				      ...
				      void *arg)
{
	...
	if (bss->state == BSS_ADMIN_DOWN && bss->previous_state == BSS_UP) {
		if (BGP_DEBUG(bfd, BFD_LIB))
			zlog_debug(
				"%s: BFD received Admin Down from peer %s - BGP session maintained",
				__func__, peer->host);
...
static void bgp_group_remove_bfd(struct peer *p)
{
	...
	/* Ensure group pointer is valid before proceeding */
	if (p->group == NULL)
		return;
	...
	if (BGP_DEBUG(bfd, BFD_LIB)) {
		if (p->bgp)
			zlog_debug("%s: removing BFD config from peer-group %s vrf %s(%u), will deregister sessions for %d peers",
				   ...
				   p->bgp->vrf_id, listcount(p->group->peer));
		else
			zlog_debug("%s: removing BFD config from peer-group %s",
				   __func__, p->group->name);
	}
	if (p->bgp)
