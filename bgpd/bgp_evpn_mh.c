...
static void bgp_evpn_local_type1_evi_route_del(struct bgp *bgp,
		struct bgp_evpn_es *es);
static struct bgp_evpn_es_vtep *bgp_evpn_es_vtep_add(struct bgp *bgp, struct bgp_evpn_es *es,
						     struct ipaddr vtep_ip, bool esr,
						     uint8_t df_alg, uint16_t df_pref, int *zret);
static enum zclient_send_status bgp_evpn_es_vtep_del(struct bgp *bgp, struct bgp_evpn_es *es,
						     struct ipaddr vtep_ip, bool esr);
	...
	*route_changed = 1;
	...
	if (remote_pi) {
		flog_err(EC_BGP_ES_INVALID,
			 ...
			 bgp->vrf_id, es ? es->esi_str : "Null", es ? &es->originator_ip : NULL);
		return -1;
	}
	...
	if (*route_changed) {
		if (BGP_DEBUG(evpn_mh, EVPN_MH_RT))
			zlog_debug("local ES %s vni %u route-type %s nexthop %pI4  %pI6 updated",
		...
		struct bgp_evpn_es *es, struct prefix_evpn *p)
{
	...
	ret = bgp_evpn_mh_route_update(bgp, es, NULL, afi, safi, dest, &attr,
				       &pi, &route_changed);
	if (ret != 0)
		flog_err(EC_BGP_ES_INVALID, "%u ERROR: Failed to updated ES route ESI: %s VTEP %pIA",
	...
	if (vpn) {
		...
		if (ret != 0)
			flog_err(EC_BGP_ES_INVALID,
				 ...
				 bgp->vrf_id, es->esi_str, vpn->vni, &es->originator_ip);
		global_rd = &vpn->prd;
	} else {
		/* EAD-ES route update */
...
static struct bgp_evpn_es_vtep *bgp_evpn_es_vtep_find(struct bgp_evpn_es *es, struct ipaddr vtep_ip)
{
...
static void bgp_evpn_path_es_unlink(struct bgp_path_es_info *es_info)
{
	...
	pi = es_info->pi;
	if (BGP_DEBUG(evpn_mh, EVPN_MH_RT))
		zlog_debug("vni %u path %pFX unlinked from es %s", es_info->vni, &pi->net->rn->p,
			   es->esi_str);
			  ...
			  uint16_t df_pref, bool bypass)
{
	...
	struct ipaddr old_originator_ip;
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("add local es %s orig-ip %pIA df_pref %u %s", es->esi_str, originator_ip,
			   df_pref, bypass ? "bypass" : "");

	// Check if originator IP has changed
	if (!new_es && !ipaddr_is_same(&old_originator_ip, originator_ip)) {
		regen_esr = true; // Trigger ESR regeneration for IP change
		regen_ead = true;
		if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
			zlog_debug("add local es %s old-orig-ip %pIA orig-ip %pIA df_pref %u %s oper_up:%d",
			      ...
			      uint32_t *nhg_p)
{
	...
	*nhg_p = 0;
	...
	/* EVPN imported IPv4 or IPv6 routes whose next hop is IPv6,
	 * it implies that the IPv6 VTEP must use an IPv6 next-hop group
	 */
...
static struct bgp_evpn_es_evi_vtep *bgp_evpn_es_evi_vtep_new(struct bgp_evpn_es_evi *es_evi,
							     struct ipaddr vtep_ip)
{
...
/* check if VTEP is already part of the list */
static struct bgp_evpn_es_evi_vtep *bgp_evpn_es_evi_vtep_find(struct bgp_evpn_es_evi *es_evi,
							      struct ipaddr vtep_ip)
{
	struct listnode *node = NULL;
	struct bgp_evpn_es_evi_vtep *evi_vtep;
				    ...
				    struct bgp_evpn_es_evi_vtep *evi_vtep)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("es %s evi %u vtep %pIA %s", evi_vtep->es_evi->es->esi_str,
							 ...
							 struct ipaddr vtep_ip, bool ead_es)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("add es %s evi %u vtep %pIA %s", evi_vtep->es_evi->es->esi_str,
			   evi_vtep->es_evi->vpn->vni, &evi_vtep->vtep_ip,
						    ...
						    const struct prefix_evpn *p)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("del remote %s es %s evi %u vtep %pIA",
			   p->prefix.ead_addr.eth_tag ? "ead-es" : "ead-evi",
	...
	ead_es = !!p->prefix.ead_addr.eth_tag;
	ret = bgp_evpn_es_evi_vtep_del(bgp, es_evi, p->prefix.ead_addr.ip, ead_es);
	bgp_evpn_es_evi_remote_info_re_eval(es_evi);
