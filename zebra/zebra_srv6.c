				     ...
				     uint32_t *sid_wide_func, const char *expected_locator_name)
{
	...
	*sid_wide_func = 0;
	...
	if (expected_locator_name && strlen(expected_locator_name) > 0) {
		...
		if (!expected_loc) {
			flog_err(EC_ZEBRA_SRV6_LOCATOR_LOOKUP_FAIL,
				 "%s: Expected locator %s not found", __func__,
		...
		if (!prefix_match((struct prefix *)&expected_loc->prefix,
			...
			flog_err(EC_ZEBRA_SRV6_INVALID_REQUEST,
				 "%s: SID %pI6 does not belong to locator %s (prefix %pFX)",
	...
	flog_err(EC_ZEBRA_SRV6_LOCATOR_LOOKUP_FAIL,
		 "%s: No expected locator name provided for explicit SID allocation", __func__);
	return false;
}
		...
		if (memcmp(&s->ctx, ctx, sizeof(struct srv6_sid_ctx)) == 0) {
			...
			if (sid_same(&s->sid->value, sid_value)) {
				...
				*sid = s->sid;
				...
				flog_err(EC_ZEBRA_SRV6_CTX_ALREADY_ASSOCIATED,
					 "%s: cannot alloc SID %pI6 for ctx %s: ctx already associated with SID %pI6",
					 __func__, sid_value,
		 ...
		 const char *locator_name)
{
	...
	if (!locator_name) {
		flog_err(EC_ZEBRA_SRV6_LOCATOR_LOOKUP_FAIL,
			 "%s: invalid SM request arguments: missing SRv6 locator, necessary for SID allocation",
			 __func__);
		return -1;
	}
	...
	if (ctx->ifindex != 0 && IPV6_ADDR_SAME(&ctx->nh6, &in6addr_any)) {
		...
		if (!ifp) {
			flog_err(EC_ZEBRA_UNKNOWN_INTERFACE, "%s: interface %u does not exist",
		...
		if (!found) {
			...
			if (!found) {
				flog_err(EC_ZEBRA_UNKNOWN_INTERFACE,
					 "%s: cannot get SID, interface (ifindex %u) not found",
					 __func__, ctx->ifindex);
				return -1;
			}
		}
					     ...
					     const char *locator_name)
{
	...
	if (!locator_name) {
		flog_err(EC_ZEBRA_SRV6_LOCATOR_LOOKUP_FAIL,
			 "%s: locator name is required for explicit SID release", __func__);
		zsend_srv6_sid_notify(client, ctx, &sid_value, 0, 0, NULL,
