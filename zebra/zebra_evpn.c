...
void zebra_evpn_read_mac_neigh(struct zebra_evpn *zevpn, struct interface *ifp)
{
	...
	if (!br_if) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug(
				"VNI %u - interface %s(%u) not mapped to bridge, skipping MAC/neigh read",
			      ...
			      struct in_addr vtep_ip)
{
	...
	if (!ifp || !if_is_operative(ifp) || !zif || !zif->brslave_info.br_if) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug(
				...
				vni);
		if (macaddr && ipaddr)
			frrtrace(5, frr_zebra, process_remote_macip_del, vni, macaddr, ipaddr,
				 vtep_ip, 1);
		return;
	}
	...
	if (n)
		...
		if (zvrf->dad_freeze
			...
			if (macaddr && ipaddr)
				frrtrace(5, frr_zebra, process_remote_macip_del, vni, macaddr,
					 ipaddr, vtep_ip, 6);
		}
