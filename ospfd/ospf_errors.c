...
static struct log_ref ferr_ospf_warn[] = {
	...
	},
	{
		...
		.suggestion = "Ensure your key is correct, gather log data from this side as well as peer and "
		"open an Issue",
	},
	{
		.code = EC_OSPF_PACKET,
		...
		.suggestion = "Ensure interface configuration is correct, gather log files from here and the "
		"peer and open an Issue",
	},
	{
		.code = EC_OSPF_LARGE_LSA,
		.title = "OSPF has determined that a LSA packet will be too large",
		.description = "OSPF has received data that is causing it to recalculate how large packets "
		"should be and has discovered that the packet size needed would be too large "
		"and we will need to fragment packets",
	...
	},
	{
		...
		.description = "During handling of a LSA, OSPF has discovered that the LSA's internal state is "
		"inconsistent",
	...
	},
	{
		...
		.description = "OSPF has received TE information that it was not expecting during normal "
		"processing of data",
		.suggestion = "Gather log data from this machine and it's peer and open an Issue",
	},
	{
		...
		.description = "During processing of a new lsa and attempting to save the lsa into the OSPF "
		"database, this process failed.",
	...
	},
	{
		...
		.description = "When processing a LSA update we have found and noticed an internal error where "
		"we are passing around a NULL pointer",
		.suggestion = "Gather data from this machine and it's peer and open an Issue",
	},
	{
		.code = EC_OSPF_EXT_LSA_UNEXPECTED,
		.title = "OSPF has received EXT information that leaves it in an "
		"unexpected state",
		.description = "While processing EXT LSA information, OSPF has noticed that the internal state "
		"of OSPF has gotten inconsistent",
		.suggestion = "Gather data from this machine and it's peer and open an Issue",
	},
	{
		.code = EC_OSPF_LSA_MISSING,
		.title = "OSPF attempted to look up a LSA and it was not found",
		.description = "During processing of new LSA information, we attempted to look up an old LSA "
	...
	},
	{
		...
		.description = "If you configure a ospf interface as P2P we should not detect more than one "
	...
	},
	{
		...
		.description = "OSPF has detected a state where we are attempting to grow a LSA but the LSA "
		"has reached it's maximum size",
...
static struct log_ref ferr_ospf_err[] = {
	...
	},
	{
		...
		.suggestion = "Check OSPF network config for any config issue, If the problem persists, "
		"report the problem for troubleshooting",
	},
	{
		.code = EC_OSPF_DOMAIN_CORRUPT,
		.title = "OSPF Domain Corruption",
		.description = "OSPF attempted to process a Router LSA but Advertising ID mismatch with link id",
		.suggestion = "Check OSPF network Database for corrupted LSA, If the problem persists, "
		"shutdown OSPF domain and report the problem for troubleshooting",
	},
	{
		.code = EC_OSPF_INIT_FAIL,
		.title = "OSPF Initialization failure",
		.description = "OSPF failed to initialized OSPF default insance",
		.suggestion = "Ensure there is adequate memory on the device. If the problem persists, report "
		"the problem for troubleshooting",
	},
	{
	...
	},
	{
		...
		.suggestion = "Restart OSPF instance, If the problem persists, report the problem for "
		"troubleshooting",
	},
	{
	...
	},
	{
		...
		.suggestion = "Too many neighbors configured on a single interface. Suggestion is to decrease "
		"the number of neighbors on a single interface/subnet",
	},
	{
		...
		.suggestion = "Check LDP sessions and restart LDP if necessary. Verify OSPF-LDP sync "
		"configuration.",
	},
	{
	...
	},
	{
		...
		.description = "OSPF Segment Routing router-info LSA uses incorrect flooding scope (should be "
		"Area scope, not AS scope)",
		.suggestion = "Change router-info flooding scope to Area flooding for Segment Routing "
		"configuration.",
	},
	{
		.code = END_FERR,
